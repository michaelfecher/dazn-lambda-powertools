...
Step Functions (SFN) client wrapper that knows how to forward correlation IDs (captured via `@michaelfecher/lambda-powertools-correlation-ids`).
...
Install from NPM: `npm install @michaelfecher/lambda-powertools-step-functions-client`
...
```js
const SFN = require('@michaelfecher/lambda-powertools-step-functions-client')
