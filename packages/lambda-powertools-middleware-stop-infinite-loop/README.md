...
This middleware is intended to be used alongside `@michaelfecher/lambda-powertools-middleware-correlation-ids`, which is responsible for collecting correlation IDs and incrementing the `call-chain-length` (i.e. the number of function invocations that are chained together) at the start of an invocation.

Because this middleware relies on `@michaelfecher/lambda-powertools-middleware-correlation-ids`, it needs to be applied **AFTER** `@michaelfecher/lambda-powertools-middleware-correlation-ids` (as seen below).
